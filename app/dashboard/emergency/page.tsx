...
export default function Emergency() {
    ...
    const [activeTab, setActiveTab] = useState<'sos' | 'helplines' | 'tips'>('sos');
    const [contacts, setContacts] = useState(DEFAULT_CONTACTS);
    const [newContact, setNewContact] = useState({ name: '', relation: '' });

    // Handle initial contacts load
    useEffect(() => {
        const saved = localStorage.getItem('safepath_contacts');
        if (saved) setContacts(JSON.parse(saved));
    }, []);

    const saveContacts = (updated: typeof contacts) => {
        setContacts(updated);
        ...
        <div className={styles.container}>
            ...
            <div className={styles.header}>
                <h1 className={`${styles.title} animate-fade-in`}>
                    <Siren size={28} className={styles.headerIcon} /> Emergency Center
                </h1>
                <p className={`${styles.subtitle} animate-fade-in delay-100`}>
                    Your safety is our priority. Use the tools below in case of emergency.
                </p>
            </div>
            ...
            {activeTab === 'sos' && (
                <div className={`${styles.tabContent} animate-fade-in`}>
                    <div className={styles.sosGrid}>
                        ...
                        {/* Trusted Contacts + Notification Status */}
                        <div className={styles.contactsCard}>
                            ...
                            <div className={styles.contactsList}>
                                {contacts.map((c) => {
                                    const isNotified = notified.some(n => n.includes(c.name));
                                    return (
                            ...
                            {!alertSent && (
                                <div className={styles.addContactForm}>
                                    <h4 style={{ fontSize: '0.85rem', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '6px' }}>
